...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("PendleConnector");
  const contract = await ContractBuilder.deploy(sharedAddresses["pendleMarketDepositHelper"], (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"]));

  await contract.deployed();
  console.log("contract deployed to:", contract.address);
  addresses["connectors"].push({
    connector: contract.address,
    connectorType: "PendleConnector",
    chainId: chainId,
  });
