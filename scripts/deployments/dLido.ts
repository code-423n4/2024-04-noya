...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("LidoConnector");
  const contract = await ContractBuilder.deploy(sharedAddresses["lido"], sharedAddresses["lidoWithdrawal"], sharedAddresses["steth"], sharedAddresses["weth"], (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"]));

  await contract.deployed();
  console.log("contract deployed to:", contract.address);
  addresses["connectors"].push({
    connector: contract.address,
    connectorType: "LidoConnector",
    chainId: chainId,
  });
