...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("CurveConnector");
  const contract = await ContractBuilder.deploy(
    sharedAddresses["convexBooster"],
    sharedAddresses["cvx"],
    sharedAddresses["crv"],
    sharedAddresses["prisma"],
    sharedAddresses["registry"](sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"])
  );
