...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("MorphoBlueConnector");
  const contract = await ContractBuilder.deploy(sharedAddresses["MorphoBlue"], (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"]));

  await contract.deployed();
  console.log("contract deployed to:", contract.address);
  addresses["connectors"].push({
    connector: contract.address,
    connectorType: "MorphoBlueConnector",
    chainId: chainId,
  });
