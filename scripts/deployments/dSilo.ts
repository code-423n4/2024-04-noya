...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("SiloConnector");
  const contract = await ContractBuilder.deploy(sharedAddresses["siloRepository"], (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"]));

  await contract.deployed();
  console.log("contract deployed to:", contract.address);
  addresses["connectors"].push({
    connector: contract.address,
    connectorType: "SiloConnector",
    chainId: chainId,
  });
