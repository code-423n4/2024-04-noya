...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("AaveConnector");
  const contract = await ContractBuilder.deploy(sharedAddresses["aaveMarket"], sharedAddresses["aaveMarketBaseToken"], (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"]));

  await contract.deployed();
  console.log("contract deployed to:", contract.address);
  addresses["connectors"].push({
    connector: contract.address,
    connectorType: "AaveConnector",
    chainId: chainId,
  });
