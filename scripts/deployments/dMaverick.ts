...
async function main() {
  ...
  const ContractBuilder = await ethers.getContractFactory("MaverickConnector");
  const contract = await ContractBuilder.deploy(
    sharedAddresses["mav"],
    sharedAddresses["veMav"],
    sharedAddresses["maverickRouter"],
    sharedAddresses["maverickPositionInspector"],
    (sharedAddresses["registry"], vaultId, sharedAddresses["swapHandler"], sharedAddresses["valueOracle"])
  );
  ...
  addresses["connectors"].push({
    ...
    connectorType: "MaverickConnector",
    chainId: chainId,
  });
