...
library PendleLpOracleLib {
     ...
     *
     */
    function getLpToAssetRate(IPMarket market, uint32 duration) internal view returns (uint256 lpToAssetRate) {
        ...
        if (state.expiry <= block.timestamp) {
            ...
            int256 tradeSize =
                (cParam.mulDown(comp.totalAsset) - state.totalPt).divDown(Math.IONE + cParam.divDown(rateHypTrade));
    ...
    function _getPYIndexCurrent(IPMarket market) private view returns (PYIndex) {
        (IPStandardizedYield SY,, IPYieldToken YT) = market.readTokens();
    ...
    function _getPtRates(IPMarket market, MarketState memory state, uint32 duration)
        private
        view
    ...
    function _calcLpPrice(int256 totalHypotheticalAsset, int256 totalLp) private pure returns (int256) {
