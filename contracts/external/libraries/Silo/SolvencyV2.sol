...
import "../../interfaces/Silo/IPriceProvidersRepository.sol";
import "../../interfaces/Silo/ISilo.sol";
import "../../interfaces/Silo/IInterestRateModel.sol";
import "../../interfaces/Silo/ISiloRepository.sol";
...
library SolvencyV2 {
    ...
    function isSolvent(ISilo pool, address _user, uint256 liquidationThresholdMultilpier) public view returns (bool) {
        ...
        (address[] memory assets, IBaseSilo.AssetStorage[] memory assetsStates) = pool.getAssetsWithState();

        (uint256 userLTV, uint256 LiquidationThreshold) = SolvencyV2.calculateLTVs(
            SolvencyParams(pool.siloRepository(), pool, assets, assetsStates, _user), TypeofLTV.LiquidationThreshold
        );
        return userLTV <= LiquidationThreshold * liquidationThresholdMultilpier / _PRECISION_DECIMALS;
    }
        ...
        returns (uint256 userLTV, uint256 LiquidationThreshold, bool isSolvent)
    {
        ...
        (userLTV, LiquidationThreshold) = SolvencyV2.calculateLTVs(
            SolvencyParams(pool.siloRepository(), pool, assets, assetsStates, _user), TypeofLTV.LiquidationThreshold
        );
        ...
        returns (uint256 currentUserLTV, uint256 secondLTV)
    {
        ...
        uint256[] memory borrowValues =
            convertAmountsToValues(priceProvidersRepository, _params.assets, totalBorrowAmounts);
        ...
        uint256 borrowAvailableTotalValue = _getTotalAvailableToBorrowValue(
            _params.siloRepository, address(_params.silo), _params.assets, _secondLtvType, collateralValues
        );
        ...
        returns (uint256 limit)
    {
        ...
        // value of theoretical debt user can have depending on TypeofLTV
        uint256 borrowAvailableTotalValue = _getTotalAvailableToBorrowValue(
            _params.siloRepository, address(_params.silo), _params.assets, _ltvType, collateralValues
        );
    ...
    function convertAmountsToValues(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            ...
            uint8 assetDecimals = IERC20Metadata(_assets[i]).decimals();
    ...
    function getUserCollateralAmount(
        ...
        uint256 assetAmount = _userCollateralTokenBalance == 0
            ...
            : _userCollateralTokenBalance.toAmount(
                totalDepositsWithInterest(
                    _assetStates.totalDeposits, _assetStates.totalBorrowAmount, _siloRepository.protocolShareFee(), _rcomp
                ),
        ...
        uint256 assetCollateralOnlyAmount = _userCollateralOnlyTokenBalance == 0
            ...
            : _userCollateralOnlyTokenBalance.toAmount(
                _assetStates.collateralOnlyDeposits, _assetStates.collateralOnlyToken.totalSupply()
            );
    ...
    function _getTotalAvailableToBorrowValue(
        ...
        for (uint256 i = 0; i < _assets.length; i++) {
            totalAvailableToBorrowValue +=
                _getAvailableToBorrowValue(_siloRepository, _silo, _assets[i], _ltvType, _collateralValues[i]);
        }
    }
}
